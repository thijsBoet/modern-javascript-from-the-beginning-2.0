const global = {
	...
	api: {
		apiKey: '90c1383062fa64fd5ec456b8b748b423',
...
async function displayMovieDetails() {
	const movieId = window.location.search.includes('=')
	...
	movies.forEach(movie => {
		...
		div.innerHTML = `
			...
			<h4 className="swiper-rating">
				<i className="fas fa-star text-secondary"></i> ${vote_average}/10
			</h4>
		`;
...
async function displaySearchResults(results) {
	...
	results.forEach(result => {
		const { title, name, poster_path, release_date, first_air_date, id, length } =
			result;
		const div = document.createElement('div');
		div.classList.add('card');
		div.innerHTML = `
            <a href="${global.search.type}-details.html?id=${id}">
                ${
                        ...
                        src="https://image.tmdb.org/t/p/w500${poster_path}"
                        class="card-img-top"
                        alt="${global.search.type === 'movie' ? title : name}"
                    />`
										: `<img
                        src="../images/no-image.jpg"
                        class="card-img-top"
...
function showAlert(message, className = 'error') {
	...
	setTimeout(() => {
		document.querySelector('#alert').removeChild(alertEl);
	}, 3000);
}
